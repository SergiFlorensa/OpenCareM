...
- Matching de dominio endurecido en chat clinico: scoring separado directo/fuzzy, filtro anti-leak por similitud debil y normalizacion de typos clinicos (`oftamologia` -> `ophthalmology`), evitando mezclas espurias entre especialidades.
...
- Smoke automatizado de conversacion nativa disponible en `app/scripts/smoke_native_chat.py` para verificar `response_mode=general`, `matched_domains=[]` y `llm_used=true` en turnos aleatorios.
- Pipeline RAG clinico con segmentacion multi-intento por subconsulta, reranking por accionabilidad clinica (filtro AUX/verbos operativos), grounding fino de citas (`section + source`) y umbrales de calidad dinamicos por subdominio (sin activar cambios de embeddings).
- Trazabilidad de fallback clinico final unificada (`clinical_answer_quality_gate=final_structured_fallback`) cuando el flujo cae a salida estructurada en modo estricto, mejorando auditoria y depuracion QA.
- Deteccion de intencion en RAG ampliada para consultas medicas operativas sobre farmacos/tratamientos, pasos/acciones, derivacion/remision, seguimiento y casos similares.
- Routing oftalmologico reforzado por sintomas oculares en lenguaje natural (`dolor ocular`, `ojo derecho/izquierdo`, `vision borrosa`, etc.) para reducir deriva a dominios genericos.
- Respuesta clinica de cara al usuario saneada: sin rutas internas (`docs/`, `/api/v1/`) ni nombres de fichero en texto final; se muestran referencias humanas de fuente.
- Pipeline RAG con verificador de evidencia tipo cross-encoder proxy, fallback lexical `keyword_only` cuando falla verificacion minima, abstencion segura configurable y reflexion iterativa de evidencialidad (ECoRAG-like) para compactar contexto antes de generacion.
- Pipeline RAG con reranking de coherencia discursiva local (RST heuristico nucleo/satelite, centering por entidad saliente y discriminador de coherencia local/LCD) para priorizar chunks operativos y reducir ruido editorial.
- Preservacion de respuesta RAG autoritativa valida (sin telemetria `llm_*` y con validacion `valid`) para evitar sobreescritura innecesaria por fallback estructurado, manteniendo reparacion evidence-first cuando la salida esta degradada.
- La capa de coherencia discursiva aplica calculos explicitos en runtime: segmentacion EDU, TextTiling por ventanas, cadenas lexicas, coherencia LSA-like y entity-grid de transiciones.
...
- RAG local con modo determinista opcional `fact-only`: corte temprano por evidencia (early-goal test), omision de segundo pase LLM en chat clinico y memoizacion de consultas con poda por subconjunto para reducir latencia y recalculo.
